...
def extract_first_lines(input_path, n=2):
    """Returns a list containing the first n lines of the file (each line still includes its trailing newline character, if any)."""
    first_lines = []
    with open(input_path, "r") as f:
...
def write_lines(output_path, lines):
    """Writes 'lines' to a new file at output_path (overwriting it if it already exists)."""
